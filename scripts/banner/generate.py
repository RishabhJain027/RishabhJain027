#!/usr/bin/env python3
"""Generate the animated GitHub profile banners for Rishabh Jain.
...
SOURCE = ROOT / "assets/source/rishabh.png"
ASSETS = ROOT / "assets"
...
ROWS = [
    ("Subject", "Rishabh"),
    ("Role", "Engineer · Cinematographer · Creative Tech"),
    ("Origin", "Mumbai · India"),
    ("Education", "SAKEC · Mumbai"),
    ("Status", "Building + Filming + Shipping"),
    ("ToolChain", "VS Code · Blender · Git"),
    ("Core.Lang", "C++ · Python · TypeScript · JavaScript"),
    ("Core.Hardware", "Arduino · ESP32 · Raspberry Pi · IoT"),
    ("Core.Frontend", "React · Next.js · Three.js · Tailwind"),
    ("Core.Creative", "GLSL · Premiere Pro · DaVinci Resolve"),
    ("Core.AI", "PyTorch · OpenCV · Node.js"),
    ("Core.Infra", "Linux · Docker · Vercel"),
    ("Grid.LinkedIn", "/in/rish-abh27"),
    ("Grid.Instagram", "@r1sh.jpeg"),
    ("Grid.GitHub", "RishabhJain027"),
]
...
def make_logos() -> dict[str, Image.Image]:
    ...
    # 1. Gear (Hardware / IoT / Engineering)
    ...
    # 3. Camera aperture / iris (Cinematographer)
    aperture = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    d = ImageDraw.Draw(aperture)
    d.ellipse((50, 50, 350, 350), fill="black")
    d.ellipse((75, 75, 325, 325), fill=(0, 0, 0, 0))
    d.ellipse((95, 95, 305, 305), fill="black")
    d.ellipse((155, 155, 245, 245), fill=(0, 0, 0, 0))
    for i in range(6):
        ang = i * math.pi / 3
        x1 = 200 + 55 * math.cos(ang)
        y1 = 200 + 55 * math.sin(ang)
        x2 = 200 + 140 * math.cos(ang + 0.65)
        y2 = 200 + 140 * math.sin(ang + 0.65)
        d.line([(x1, y1), (x2, y2)], fill=(0, 0, 0, 0), width=8)
    logos["aperture"] = aperture
...
def portrait_points(theme: str, rng: np.random.Generator) -> np.ndarray:
    ...
    crop = source.crop((left, 0, left + target_w, h)).resize((300, 340), Image.Resampling.LANCZOS)
...
def render_svg(
    ...
    aperture = transport(code, logo_points["aperture"][:n])

    times = [0, 3.0, 4.3, 6.3, 7.6, 9.6, 10.9, 12.9, 14.2]
    key_times = ";".join(num(v / LOOP_SECONDS) for v in times)
    frames = [source, source, gear, gear, code, code, aperture, aperture, source]
    ...
    parts: list[str] = [
        ...
        '<title id="title">Rishabh live system profile</title>',
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        'Gear, Code, and Aperture silhouettes.</desc>',
        '<defs>',
    ...
    parts.extend(
        [
            ...
            'font-family="ui-monospace,SFMono-Regular,Consolas,monospace" font-size="13" '
            'font-weight="700">@RishabhJain027</text>',
        ]
    )
